...
## Shiny Tab1 - Visualizatoin
* Visualization displays selected variety in a well formed viz;
* Vertical line to indicate training/validation split
* Training interval (cutoff_date) is controllable
...
fit |> 
  ...
  tab_header(title = "Training Accuracy (Training Set)")
```
...
* Select forecast horizon (default 12 months)
* Display forecast visualization (the two plots below)
...
```{r}
# Forecast Table — use case-insensitive model match and format Month for headers
fc |>
